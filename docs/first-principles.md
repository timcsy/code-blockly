...
**lift() 是語用分析的確定性近似器**——它用 pattern matching 和上下文查表來近似語用推斷。目前覆蓋的主要是慣用語辨識（idiom recognition）和上下文消歧，尚未觸及真正的意圖推斷（「為什麼在這裡寫這個」）。真正的語用推斷是機率性的，由 LLM 層處理（§4.3），lift() 只負責確定性可驗證的部分：
