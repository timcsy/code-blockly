...
每一層獨立可配置、獨立可擴充。維度集合本身也是開放的——如果發現現有維度無法覆蓋的正交配置需求，可追加新維度。View Params 的內容隨 View Type 而變——Code Style、Block Style、Layout Algorithm 等都是特定 viewType 的 View Params。User Context 不直接影響投影結果，而是作為 View Params 的**推導來源**——當使用者沒有明確指定參數時，系統從慣性快取和角色意圖中推導預設值。
...
```
積木是否存在於 Toolbox = f(P4 概念層級, View Params)

其中：
  P4 概念層級（Level）  → 控制「哪些概念可見」（可見性）
...
**關鍵區分**：`print` 概念在 L0 就可見（P4 決定），但投影為 cout 還是 printf 是 Code Style 的 View Params 決定的。這不是 P4 的多維度化——P4 只管「概念是否可見」，View Params 管「可見概念如何投影」。兩者正交，不應混合。
