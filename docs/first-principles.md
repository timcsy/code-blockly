...
| `comment` | 註解（語義樹的一部分） | **R0** | 註解節點本身就在語義結構中 |
...
```
R0 雙射 ──────── code:same, blocks, ast_tree, comment, semantic_package, share_link
R1 語義保留 ──── code:restyle, scaffolded, syntax_highlight, braille,
                  high_contrast, embed_snippet, flowchart(簡單情況)
R2 結構可逆 ──── pseudocode, state_machine, data_flow, screen_reader,
...
```
┌─ Scope ─────────────────── 觀察範圍（語句→系統）──────────────┐
├─ View Type ────────────── 投影種類（七族：文字/視覺/行為/...）──┤
├─ View Params ──────────── 投影參數（每種 viewType 各自的參數）──┤
├─ Concept Layer ─────────── 概念代數（分層、組合、映射）─────────┤
├─ Language Layer ────────── 語言專屬（型別、語法、慣例）─────────┤
├─ Locale ───────────────── 自然語言（zh-TW / en / ...）────────┤
└──────────────────────────────────────────────────────────────┘
```

每一層獨立可配置、獨立可擴充。View Params 的內容隨 View Type 而變——Code Style、Block Style、Layout Algorithm 等都是特定 viewType 的 View Params。
...
| **block style** | P1：Block Style 是 `blocks` viewType 的 viewParams。S4：預設值符合學習者閱讀習慣 |
| **多語言支援** | P2：區分三層概念。跨語言走 abstract concept 映射（R2-R3 投影） |
...
**獨特定位**：不是在語義層之上疊加積木、程式碼、執行，而是**只有語義層，其他都是投影**。七個投影族（文字、視覺結構、行為、分析、教育、可及性、分發）統一為同一個投影管線，可逆性從 R0（雙射）到 R4（單向）分級。
...
14. **Pattern 依賴的教育反噬**：lift() 的 pattern 匹配機制隱性地建立了「獎勵函數」——匹配 pattern 的程式碼獲得精確積木（視覺獎勵），不匹配的降級為 raw_code（視覺懲罰）。這可能導致學習者為了「讓積木顯示出來」而被迫寫出符合系統 pattern 的程式碼，限制探索和創造力。對策：降級視覺必須區分原因（錯誤 / 系統能力不足 / 非標準但正確），使「系統不認識」不等於「你寫錯了」。長期方向：LLM 輔助的語用分析可擴大 pattern 覆蓋率，降低「能力不足」型降級的比例。
