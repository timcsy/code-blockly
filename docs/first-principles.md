...
> **降級不是失敗，而是系統選擇保留原始資訊而非強行結構化的正確行為。** 系統的職責不是消除降級，而是讓降級透明可見、原因可區分、資訊不丟失。
...
P4（漸進揭露）的本質是：**語義結構始終完整，只有可見集合隨脈絡改變**。所有的「選擇性呈現」都可以統一理解為遮罩函數 `mask(context) → VisibleSet`：

| 遮罩類型 | context 參數 | 已實現 |
|---------|-------------|--------|
| **教學遮罩** | 學習者的 enabledBranches | 是（Topic Level Tree） |
| **風格遮罩** | Code Style 偏好 | 是（Style Preset） |
| **鷹架遮罩** | Program Scaffold 層級 | 是（L0 隱藏→L1 ghost→L2+ 手動） |
| **ZPD 遮罩** | 學習者的近側發展區（自動偵測） | 否（未來） |
| **領域遮罩** | 跨學科的語義投射 | 否（未來） |
| **多維投影** | viewType（積木/程式碼/流程圖/敘事） | 部分（積木+程式碼） |

**與 Transformer 的類比**：固定的 L0→L1→L2 線性解鎖類似 RNN 的步進遞迴（容易卡在局部最佳解）。現在的 Topic + enabledBranches 架構類似 Transformer 的注意力遮罩——結構從一開始就完整存在，遮罩決定當前可見的子集。多維投影（同一語義樹同時投射為積木、程式碼、流程圖、敘事）類似 Multi-head Attention，提供多角度的認知理解。

### 自蒸餾：教學路徑的遞迴優化

當系統不斷優化其層級樹（P4）時，它實際上是在對自身進行知識壓縮——AI 領域稱之為**自蒸餾（Self-distillation）**：

```
第 1 代層級樹：教育專家手動設計
     ↓ 累積 User Context（學習軌跡、成功率、ZPD 數據）
第 2 代層級樹：系統根據數據優化分支結構
     ↓ 進一步壓縮
第 N 代層級樹：以更小的結構表達同樣複雜的邏輯
```
...
**教學套件化**：優秀的教學路徑可以被封裝為語義套件（§4.2），跨語言、跨 Topic 共享。這使得教學經驗本身成為可複用的語義結構。
...
自舉有一個邏輯極限：系統可以優化所有分支，但**根節點必須由人類預設**。這不是缺陷，而是系統的完備性邊界——類似哥德爾不完備定理，系統無法無中生有地定義「什麼是基礎」。

這自然導出三層責任分離：

```
語義結構（System）  ── 負責「真（Truth）」與「健全（Soundness）」
                      確定性、可驗證、不可變

語用推斷（LLM/AI）  ── 負責「像（Likelihood）」與「慣例（Convention）」
                      機率性、可建議、需確認（§4.3）

人類意圖（Human）   ── 負責「美（Aesthetics）」與「價值（Value）」
                      定義根公理、判斷 correctness（§4.3 Soundness vs Correctness）
```

**LLM 作為導師投影**：在這個框架下，LLM 不再是漫無目的地回答問題，而是作為語用分析師（§4.3），分析學習者目前的語義樹狀態，並精確地投影出下一條最適合的學習引導（Hint Projection）。系統管理的複雜度越高，人類的品味——「定義問題的品味」——就越重要。

**種子層級**：最初的根公理（`docs/first-principles.md` 本身、Topic Level Tree 的根節點）仍需依賴人類的直覺和教育專業，直到系統累積足夠的 User Context 來以數據驗證哪些概念才是真正的認知起點。這呼應了 Sc2（積木不可引入程式碼中不存在的概念）——系統不能無中生有。
...
> **程式是語義結構——檔案內是樹、檔案間是圖、系統間是超圖。程式碼、積木、流程圖、執行、敘事、分析、分發都是它的參數化投影，由 scope 和 viewType 決定觀察角度，可逆性從 R0（雙射）到 R4（單向）分級。概念形成可分層、可映射的代數結構，可封裝為語義套件透過可插拔後端執行與共享。LLM 作為語用分析師在確定性 guardrails 之上輔助推斷與生成。系統透過開放擴充成長，透過漸進揭露（主題情境 × 概念層級樹 × 結構範圍）適應從初學者到系統架構師、從課堂教學到嵌入式開發到競賽程式的所有使用者與情境。積木是認知鷹架——降低外在負荷、在近側發展區內引導學習，並最終退場。**
