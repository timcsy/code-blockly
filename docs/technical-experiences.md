...
本文件記錄 semorphe 開發過程中累積的架構知識、設計模式、從第一性原理到實作過程中遇到的真實問題與解法，供後續開發或新語言擴充時參考。
...
7. [屬性結構化 vs 字串壓扁的臨界點](#7-屬性結構化-vs-字串壓扁的臨界點)
8. [雙角色概念與語境切換](#8-雙角色概念與語境切換)
9. [依賴解析的接線與定位](#9-依賴解析的接線與定位)

**Part II — 架構知識與模式**

10. [積木定義的兩層架構](#10-積木定義的兩層架構)
11. [程式碼產生的優先順序](#11-程式碼產生的優先順序)
12. [Lifting（程式碼→積木）](#12-lifting程式碼積木)
13. [積木提取（Blockly→Semantic）](#13-積木提取blocklysemantic)
14. [Concept 與 Registry](#14-concept-與-registry)
15. [i18n 國際化](#15-i18n-國際化)
16. [Toolbox 與認知層級](#16-toolbox-與認知層級)
...
P2 概念代數定義了 SemanticNode 的 properties 為 `Record<string, PropertyValue>`，但未規定 PropertyValue 何時該用字串、何時該用結構化物件。

### 實作中發生了什麼

**真實案例：`long long` 參數型別被截斷**

`func_def` 的 params 存成字串陣列 `["long long base", "int y"]`。renderer 用 `split(/\s+/)` 拆分，將 `"long long base"` 拆為 `["long", "long", "base"]`，TYPE 欄位只拿到第一個 token `"long"`。

這是**把結構化資訊壓扁成字串再反向解析**的典型反模式。同一份字串在四個環節被不同方式解讀：

```
lifter:    param.text → "long long base"     ← 整串存入
generator: params.join(', ')                  ← 正確（照原樣拼接）
renderer:  split(/\s+/)[0] → "long"          ← 壞掉
extractor: TYPE_i + PARAM_i → 正確（從 UI 欄位讀）
```

### 如何解決

短期：用已知複合型別清單做前綴匹配（`parseParamTypeAndName`），從最長候選開始匹配。
長期：params 應結構化為 `[{ type: "long long", name: "base" }]`。

### 教訓

**判斷準則：如果一個字串值需要被 parse 回結構才能正確使用，它就不該是字串。** 壓扁為字串的唯一合理場景是：該值只會被完整傳遞、不會被拆分。一旦下游需要拆分，就必須在上游保持結構化。
...
P2 的 ConceptDef 定義了 `role: "statement" | "expression"`，隱含一個概念只有一個角色。

### 實作中發生了什麼

C++ 的 `cin >> a >> n >> m` 既可以是 statement（獨立語句），也可以是 expression（`if` 條件中）。同一個語義概念 `input` 需要兩種角色。

**問題一：積木版本分裂**

statement 版 `u_input`（有 prev/next 連接器）不能嵌入 expression 插槽。必須建立 expression 版 `u_input_expr`（有 output 連接器）。block-renderer 透過 `STATEMENT_TO_EXPRESSION` 映射表在 expression context 自動替換。

**問題二：generator 需要知道自己處於哪個上下文**

`input` generator 在 statement context 產出 `cin >> a >> n >> m;\n`（含 indent 和分號），在 expression context 只需產出 `cin >> a >> n >> m`。解法：`GeneratorContext` 加入 `isExpression` flag，`generateExpression()` 設定 `{ ...ctx, isExpression: true }`。

**問題三：extraState 契約不一致**（最隱蔽的 bug）

`u_input` 的 `saveExtraState` 輸出 `{ mode: 'select', text: 'a' }`。`u_input_expr` 是從 `u_input` 複製貼上再改的，`loadExtraState` 讀的是 `slot.selectedVar` 而非 `slot.text`。`STATEMENT_TO_EXPRESSION` 映射直接把 statement 版的 BlockState（含 extraState）套到 expression 版上——欄位名不一致導致所有變數都 fallback 成預設值 `base`。

### 如何解決

1. **`isExpression` flag**：乾淨的 context 傳遞，generator 自行判斷是否加 indent/分號
2. **extraState 契約統一**：同一概念的 statement 和 expression 版本，`saveExtraState` / `loadExtraState` 格式**必須完全相同**
3. **`STATEMENT_TO_EXPRESSION` 映射**：renderer 層面集中管理角色切換，不汙染 generator 邏輯

### 教訓

**複製貼上積木定義是序列化契約分裂的溫床。** 當一個概念需要 statement/expression 兩個版本時，它們的序列化格式是隱式的 API 契約——`STATEMENT_TO_EXPRESSION` 映射假設兩者的 extraState 完全相容。違反這個假設不會在編譯期報錯，只會在特定語境（如 if 條件中的 cin）才暴露。

## 9. 依賴解析的接線與定位

### 第一性原理說了什麼

§1.3 指出 `#include` 是 Scope 3 圖邊在 Scope 2 的投影——可從語義樹的概念使用推導出來的衍生資訊。§2.1 P1 要求投影管線分離。§2.3 P3 要求依賴解析是語言模組的職責，核心只定義介面。
...
但 UI 層（`app.ts`）從來沒有呼叫 `setModuleRegistry()`。`generateCodeWithMapping()` 建構 `GeneratorContext` 時 `moduleRegistry` 永遠是 `undefined`，auto-include 的 `if (ctx.moduleRegistry)` 永遠不進去。

所有單元測試都通過（因為測試手動注入了 registry），但瀏覽器中功能完全無效。

**問題二：ModuleRegistry 用錯欄位名**

`ConceptDefJSON` 的欄位是 `conceptId`（不是 `id`）。`ModuleRegistry.register()` 寫成 `concept.id`，TypeScript 的 structural typing 沒報錯（因為 `id` 在某些型別中是合法屬性），registry 靜默地註冊了一堆 `undefined` 作為 key。

**問題三：Generator 混入依賴解析**

program generator 同時負責「產生程式碼」和「推導需要哪些 #include」，違反了 P1 投影管線分離。正確定位：DependencyResolver 是 Scope 3 索引構建器，不是 Scope 2 generator 的一部分。

### 如何解決

1. 加入全域 `setModuleRegistry()` + 在 `app.ts` 初始化時呼叫 `setModuleRegistry(createPopulatedRegistry())`
2. 修正 `concept.id` → `concept.conceptId`
3. Generator 中的 auto-include 邏輯暫留（功能正確），但認識到架構上應抽離為獨立的 Scaffold 層

### 教訓

**端到端驗證不可省。** 單元測試手動注入依賴，繞過了真實的接線路徑。如果沒有瀏覽器端的手動驗證（或端到端測試），「功能寫好了但沒接上」的問題可以存活很久。**全域 setter 模式的風險**：`setModuleRegistry` / `setTemplateGenerator` 這種隱式全域注入，呼叫順序和遺漏都無法被型別系統捕獲。
...
除了精確匹配（`value: "++"`），constraints 支援前綴匹配：

```jsonc
{ "field": "$text", "match": "startsWith", "text": "/**" }
```

真實案例：區分 `/** doc comment */` 和 `/* block comment */`——兩者的 tree-sitter nodeType 都是 `comment`，用 `startsWith` constraint 區分前綴。Pattern 定義順序很重要：具體的（`/**`）必須排在寬泛的（`/*`）前面。

### 常見陷阱

**陷阱一：空 constraints 搶匹配**
...
解法：務必加上 constraints 限縮匹配範圍，或改用 hand-written lifter。

**陷阱二：liftStrategy 在 constraint 之前執行**

早期實作中，pattern-lifter 先執行 `liftStrategy`（hand-written 函式），成功後才檢查 constraint。結果 `// 單行註解`（nodeType 也是 `comment`）也會跑進 `cpp:liftDocComment` strategy，產出錯誤的 doc_comment。

修正：**constraint-before-strategy** 是不變式。先驗證 constraint 通過，才執行 liftStrategy。這保證 strategy 函式只處理已經確認屬於自己的 AST 節點。

---

## 13. 積木提取（Blockly→Semantic）
...
### 預配置變體（extraState Presets）

同一個積木類型可以在 toolbox 中以不同 `extraState` 出現，展示概念的不同結構複雜度：

```typescript
const extraReplacements = new Map([
  ['u_if', [
    { type: 'u_if' },                                         // 如果
    { type: 'u_if', extraState: { hasElse: true } },           // 如果/否則
    { type: 'u_if', extraState: { elseifCount: 1, hasElse: true } }, // 如果/否則如果/否則
  ]],
])
```

搭配 `excludeTypes` 排除不需要的舊版積木（如 legacy `u_if_else`），避免重複。每個變體可設 `level` 控制在哪個認知層級才顯示。

---

## 17. 動態積木常用模式

### 16.1 `const self = this` 模式
...
### 16.4 三模式參數（select/compose/custom）
...
**Mutator 內的文字也要遵守第一性原理。** Container 和 Item block 的 label 不是「實作細節」——使用者會看到它們。例如 doc comment 的 mutator 用「參數」和「回傳」而非 `@param` 和 `@return`。

### 16.6 Statement↔Expression 雙版本

當一個概念同時可作為 statement 和 expression 時（如 `input`/`increment`/`scanf`），需要兩個積木版本：

```typescript
// block-renderer.ts 集中管理映射
...
關鍵約束：
- 兩個版本的 `saveExtraState` / `loadExtraState` 格式**必須完全相同**（映射時直接搬移 extraState）
- generator 用 `ctx.isExpression` 判斷是否加 indent/分號
- 只需要一個 `extractBlockInner` case（兩個版本共用同一個 extract 函式）

### 16.7 FieldMultilineInput 外掛

Blockly 12.x 將 `FieldMultilineInput` 移至獨立套件 `@blockly/field-multilineinput`。使用時需要：

```typescript
import { FieldMultilineInput } from '@blockly/field-multilineinput'
// 在 init() 中使用
input.appendField(new FieldMultilineInput('') as Blockly.Field, 'TEXT')
```

PatternRenderer 的 `deriveRenderMapping` 也需要辨識 `field_multilinetext` 類型。
...
| mutator extraState 格式不相容 | 舊存檔載入後積木形狀錯誤 | `loadExtraState` 無法解析新格式 | 直接升級格式、刪除舊路徑；`??` 預設值是 null safety 而非向後相容 |
...
| 複合型別被截斷 | `long long` 變成 `long` | `split(/\s+/)` 只取第一個 token | 用已知複合型別清單做前綴匹配，或結構化存儲 |
| statement/expression extraState 不一致 | expression 版積木的欄位全部 fallback 為預設值 | 複製貼上時 `loadExtraState` 用了不同的欄位名 | 同概念雙版本的序列化格式必須完全相同 |
